...
import { getAllTeamsLineupData, LineUpData } from '../eventServices';
...
const ConfirmedPairsInfo = () => {
  ...
  const fetchTeamsLineup = useCallback(async (round: string) => {
    try {
      const response = await getAllTeamsLineupData({ season: 1, round });
  ...
  useEffect(() => {
    fetchTeamsLineup(activeRound);
    return () => { }
  }, [activeRound, fetchTeamsLineup])
...
export default ConfirmedPairsInfo